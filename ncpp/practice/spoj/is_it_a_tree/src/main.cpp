...
/** a graph would be a tree, if:
  1. it would have no cycle
  2. no. of connected components is 1 */
...
void dfs(int n) {
	visited[n] = 1;
	for(const int& x: nodes[n]) if(!visited[x]) dfs(x);
}

void solvethetestcase() {
	bool is_it_a_tree = true;
	int n{}, e{};
	cin >> n >> e;
	if(e <= (n - 1)) {  /** a tree would have max of n - 1 edges */
		int a{}, b{};
		while(e--) {
			cin >> a >> b;
			nodes[a].pb(b), nodes[b].pb(a);
		}
		int cc{0};
		for(int i = 1; i <= n; i++) {
			if(!visited[i]) dfs(i), cc++;
		}
		if(1 < cc) is_it_a_tree = false;
	}
	else is_it_a_tree = false;
	cout << (is_it_a_tree ? "YES" : "NO") << nl;
}
