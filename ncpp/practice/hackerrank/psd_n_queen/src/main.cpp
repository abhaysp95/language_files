...
bool is_safe(const vvi& t, int row, int col) {
	// check columns
	rep(i, row) {
		if(1 == t[i][col]) return false;
	}

	// check left & right diagonal(above)
	int x = row, y = col;
	while(0 <= x && 0 <= y) {
		if(1 == t[x][y]) return false;
		x--;
		y--;
	}
	x = row;
	y = col;
	while(0 <= x && t[0].size() > y) {
		if(1 == t[x][y]) return false;
		x--;
		y++;
	}
...
bool place_queen(vvi& t, int row) {
	if(t.size() == row) {
		return true;
	}
	loop(i, 0, t.size()) {
		if(is_safe(t, row, i)) {
			t[row][i] = 1;
			if(place_queen(t, row + 1)) return true;
			t[row][i] = 0;
		}
	}
	return false;
