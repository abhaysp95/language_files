...
/** diff between longest common substring and subsequence */
...
int get_lcsq( const string& a, const string& b ) {
	rloop( i, 1, a.size() ) {
		rloop( j, 1, b.size() ) {
...
int get_lcst( const string& a, const string& b ) {
	int res = -INF;
	rloop( i, 1, a.size() ) {
		rloop( j, 1, b.size() ) {
			if( a[i - 1] == b[j - 1] ) {
				//cout << "[i, j]: " << i << ", " << j << "c: " << a[i] << nl;
				t[i][j] = 1 + t[i - 1][j - 1];
				res = rmx( res, t[i][j] );
			}
		}
...
void solvethetestcase() {
	...
	cin >> a >> b;
	t.clear();
	t.resize( a.size() + 1, vi( b.size() + 1, 0 ) );
	// get subsequence( longest common )
	int lcsq = get_lcsq( a, b );
	//cout << lcsq << nl;
	//print_mat( t );

	// get substring( longest common )
	t.clear();
	t.resize( a.size() + 1, vi( b.size() + 1, 0 ) );
	int lcst = get_lcst( a, b );
	//cout << lcst << nl;
	//print_mat( t );
	cout << lcsq - lcst << nl;
}
