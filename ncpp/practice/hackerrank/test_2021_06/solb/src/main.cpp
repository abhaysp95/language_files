...
bool is_safe( const vvi& g, int i , int j ) {
	if( 0 <= i && g.size() > i && 0 <= j && g[0].size() > j && 1 == g[i][j] ) return true;
	return false;
}

bool get_path( const vvi& g, int i, int j ) {
	if( ( g.size() - 1 == i ) && ( g[0].size() - 1 == j ) ) {
		t[i][j] = 1;
		return true;
	}
	if( is_safe( g, i, j ) ) {
		t[i][j] = 1;
		if( get_path( g, i + 1, j ) ) return true;
		if( get_path( g, i, j + 1 ) ) return true;
		t[i][j] = 0;
		return false;
	}
	return false;
