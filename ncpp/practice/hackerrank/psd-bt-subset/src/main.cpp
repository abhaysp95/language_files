...
void solve( string in, string out, vs& vec ) {
	if( in.empty() ) {
		if( out.size() > 1 ) {
			string temp{};
			for( size_t i = 0; i < out.size() - 1; i++ ) {
				temp += ( string{ out.at(i) } + "," );
			}
			temp += string{ out.at( out.size() - 1 ) };
			vec.pb( "[" + temp + "]" );
		}
		else {
			vec.pb( "[" + out + "]" );
	...
	string aout{ out + in.at(0) };
	in.erase(0, 1);
	solve( in, out, vec );
	solve( in, aout, vec );
}

void solution( string in, string out, vs& vec ) {
	if( in.empty() ) {
		vec.pb( out );
		return;
	}
