...
int solve( const vi& t, int i, int j, int k ) {
	if( k == t.size() ) {
		return INF;
	}
	int asum{}, bsum{};
	int res = solve( t, i + 1, j + 1 , k + 1 );
	asum = accumulate( t.begin(), t.begin() + i + 1, 0 );
	bsum = accumulate( t.begin() + k, t.end(), 0 );
	int max_is = mx( asum, ( t.at( j ), bsum ) );
	return mn( max_is, res );
}
...
void solvethetestcase() {
	...
	cin >> count >> mason;
	bool status = false;
	if( count >= mason && count <= 10 ) {
		string temp{};
		cin.ignore();
		getline( cin, temp );
		tokenize( temp, areas, ' ' );
		for( const int& x: temp ) {
			if( 100 < x ) status = true;
		}
		if( status ) {
			cout << "Optimal solution is not possible" << nl;
		}
		// add constraint
		cout << solve( areas, 0, 1, 2 ) << nl;
	}
	else {
		cout << "Optimal solution is not possible" << nl;
	}
