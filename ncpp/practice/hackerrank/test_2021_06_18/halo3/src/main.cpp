...
bool compare_start_end( const string& a, const string& b ) {
	if( a.at(0) == b.at(0) || a.at( a.size() - 1 ) == b.at( b.size() - 1 ) ) return true;
	return false;
}

bool solve( const vs& t ) {
	string res{ t.at( 0 ) };
	bool match_found = false;
	size_t count{ 0 };
	char to_match = t.at( 0 ).at( t.at( 0 ).size() - 1 );
	while( count < t.size() - 1 ) {
		match_found = false;
		for( size_t i = 0; i < t.size(); i++ ) {
			if( to_match == t.at( i ).at( 0 ) ) {
				if( i != 0 ) {
					res += t.at( i );
					count++;
					to_match = t.at( i ).at( t.at( i ).size() - 1 );
					match_found = true;
					break;
				}
			}
	...
	if( match_found ) {
		if( res.at( 0 ) != res.at( res.size() - 1 ) ) match_found = false;
	}
	return match_found;
}
...
void solvethetestcase() {
	...
	if( 2 <= n && n <= 12 ) {
		...
		rep( i, n ) {
			cin >> temp;
			if( 2 <= temp.size() && temp.size() <= 5 ) {
				if( find( birds.begin(), birds.end(), temp ) == birds.end() ) {
					for( const string& str: birds ) {
						if( str.at(0) == temp.at(0) || str.at( str.size() - 1 ) == temp.at( temp.size() - 1 ) ) {
							error = true;
							break;
						}
					}
	...
	if( error ) {
		cout << "INVALID BIRD" << nl;
	}
	else {
		cout << ( solve( birds ) ? "YES" : "NO" ) << nl;
	}
}
