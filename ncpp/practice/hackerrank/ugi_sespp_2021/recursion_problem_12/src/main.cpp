...
size_t get_count_of_one(const string& num) {
	size_t count{};
	for(const char& c: num) if(c == '1') count++;
	return count;
}

void solve(size_t in, size_t num_ones, string num) {
	// base case
	if(num.size() == in) {
		cout << num << nl;
		return;
	}
	//int num_ones = get_count_of_one(num);
	solve(in, num_ones + 1, num + "1");
	if((num.size() - num_ones) < num_ones)
		solve(in, num_ones, num + "0");
}
