...
bool is_balanced(string& in) {
	if(')' == in[0] || '(' == in[in.size() - 1]) return false;
	int c{};
	loop(i, 0, in.size()) {
		if(0 > c) return false;
		if('(' == in[i]) c++;
...
void solve(string& in, int idx) {
	if((in.size() - 1) == static_cast<size_t>(idx)) {
		if(is_balanced(in)) {
			if(vecs.end() == find(vecs.begin(), vecs.end(), in))
				vecs.pb(in);
		}
		return;
	}
	loop(i, idx, in.size()) {
		if(in[i] == in[idx] && i != idx) continue;
		if(is_balanced(in)) {
			swap(in[i], in[idx]);
			solve(in, idx + 1);
			swap(in[i], in[idx]);
...
void solvethetestcase() {
	int count{};
	cin >> count;
	string in(string(count, '(') + string(count, ')'));
	solve(in, 0);
	vs::const_iterator citer = vecs.cbegin();
	cout << '[';
	while(vecs.cend() != (citer + 1)) cout << '"'
		<< *citer++ << '"' << ',';
	cout << '"' << *citer << '"' << ']' << nl;
}
