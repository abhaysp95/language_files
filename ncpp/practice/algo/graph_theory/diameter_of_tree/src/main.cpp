...
int dfs(int node, int& fnode) {
	visited[node] = 1;
	int size{-1};
	for(const int& x: nodes[node]) {
		if(!visited[x]) {
			int temp = 1 + dfs(x, fnode);
			if(size < temp) {
				size = temp;
				fnode = x;
			}
...
int dfs(int node) {
	...
	int size{-1};
	for(const int& x: nodes[node]) {
		if(!visited[x]) size = max(size, 1 + dfs(x));
	}
	return (-1 == size ? 0 : size);
}
...
// run dfs from any node and get the farthest node from it, now run dfs from
// this farthest node and the distance to the farthest node from this node will
// be the diameter of the tree

void solvethetestcase() {
	int n{}, e{}, a{}, b{};
	cin >> n >> e;
	nodes.clear(), nodes.resize(SZ, vi{});
	visited.clear(), visited.resize(SZ);
	while(e--) {
		cin >> a >> b;
		nodes[a].pb(b);
		nodes[b].pb(a);
	...
	int farthest_node{0};
	dfs(1, farthest_node);
	cout << "farthest node: " << farthest_node << nl;
	visited.clear(), visited.resize(SZ);
	cout << "diameter of tree: " << dfs(farthest_node) << nl;
}

/** using novice approach */

// make DFS run for every node and record the max distance from n-nodes, that
// would be the diameter of tree

/*void solvethetestcase() {
	int n{}, e{}, a{}, b{};
	cin >> n >> e;
	if(nodes.empty()) nodes.resize(SZ, vi{});
	if(visited.empty()) visited.resize(SZ);
	while(e--) {
		cin >> a >> b;
		nodes[a].pb(b);
		nodes[b].pb(a);
	}
	//cout << "max no. of edge from 1: " << dfs(1) << nl;
	int diameter = -INF;
	rloop(i, 1, n) {
		if(!visited.empty()) {
			visited.clear();
		...
		int temp = dfs(i);
		cout << "max no. edges from " << i << ": " << temp << nl;
		diameter = max(diameter, temp);
	}
	cout << "diameter of tree: " << diameter << nl;
}*/
