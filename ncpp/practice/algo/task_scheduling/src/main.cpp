...
/** you are provided with some number of tasks and start and end time for those
 * tasks is given as input. Your job is to find the schedule that would have
 * max number of tasks */

/** best idea would be select the event which ends earliest, instead of doing
 * shortest event, or doing the event which starts first */

void print_mat(const vvi& mat) {
	loop (i, 0, mat.size()) {
		loop (j, 0, mat[0].size()) cout << mat[i][j] << ' ';
		br;
	}
}

vvi transpose(vvi&& vect) {
	vvi transposed(vect[0].size(), vi());
	loop (i, 0, vect.size()) {
		loop (j, 0, vect[0].size()) {
			transposed[j].pb(vect[i][j]);
		}
	}
	return transposed;
}

int max_task_count(const vvi& data) {
	int count{1};  // first task will be selected by default
	int fend = data[1][0];  // first value of end row
	loop (i, 1, data[0].size()) {
		if (fend <= data[0][i]) {
			count++;
			fend = data[1][i];
		}
...
void solvethetestcase() {
	...
	vvi&& transposed = transpose(std::move(data));
	// now sort this transposed matrix as per second column
	sort(transposed.begin(), transposed.end(), [](vi& d1, vi& d2) {
			return d1.at(1) < d2.at(1);
			});
	data = transpose(std::move(transposed));
	//print_mat(data);
	cout << max_task_count(data) << nl;
}
