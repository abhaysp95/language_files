...
int max_sum_k_ele(const vi& arr, int k) {
	if(k > arr.size()) return -1;
	int sum{}, max_sum = -INF;
	sum = accumulate(arr.begin(), arr.begin() + k, sum);  // initial sum for k elements
	if(k == arr.size()) return sum;
...
int max_sum_k_ele2(const vi& arr, int k) {
	if(k > arr.size()) return -1;
	int sum{}, max_sum = -INF;
