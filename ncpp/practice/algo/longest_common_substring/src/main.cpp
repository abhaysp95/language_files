...
int lcs(string& a, string& b, int x, int y, int count) {
	if (!x || !y)
		return count;
	if (a[x - 1] == b[y - 1])
		count = lcs(a, b, x - 1, y - 1, count + 1);
	return mx(count, mx(lcs(a, b, x - 1, y, 0), lcs(a, b, x, y - 1, 0)));
}
...
void solvethetestcase() {
	...
	//cout << lcs(a, b, a.size(), b.size(), 0) << nl;
	cout << lcs_tab(a, b) << nl;
	print_mat();
}
