...
void print_matrix( const vvi& mat ) {
	loop( i, 0, mat.size() ) {
...
void solve( const vvi& t, vvi& d, int i, int j, long& maxm, long maxt, const size_t& tr, const size_t& tc) {
	// base_condition
	if( tr - 1 == i && tc - 1 == j ) {
		rmx( maxm, maxt );
		return;
	}
	if( is_safe( t, d, std::move( i ), std::move( j ) ) ) {
		d[i][j] = 1;
		std::cout << "==> ( " << i << ", " << j << " )" << nl;
		solve( t, d, i, j - 1, maxm, maxt + 1, tr, tc );
		solve( t, d, i + 1, j, maxm, maxt + 1, tr, tc );
		solve( t, d, i, j + 1, maxm, maxt + 1, tr, tc );
		solve( t, d, i - 1, j, maxm, maxt + 1, tr, tc );
		d[i][j] = 0;
	}
...
void solvethetestcase() {
	...
	print_matrix( t );
	vvi d;
	d.resize( t.size(), vi( t[0].size(), 0 ) );
	//print_matrix( d );
	long maxm = -INF;
	solve( t, d, 0, 0, maxm, 0, tr, tc);
	cout << maxm << nl;
}
