...
void print_mat(const vvi& matrix, int x, int y) {
	rep(i, x) {
		cout << "{";
		rep(j, y) {
			cout << matrix[i][j];
...
bool solve_maze(const vvi& gmat, vvi& rmat, int i, int j) {
	cout << "(" << i << "," << j << ")" << nl;
	if (i == gmat.size() - 1
			&& j == gmat[0].size() - 1) {
		rmat[i][j] = 1;
	...
	if (is_safe(gmat, i, j)) {
		rmat[i][j] = 1;
		if (solve_maze(gmat, rmat, i + 1, j)) {
			cout << "--> (" << i << "," << j << ")" << nl;
			return true;
		}
		if (solve_maze(gmat, rmat, i, j + 1)) {
			cout << "--> (" << i << "," << j << ")" << nl;
			return true;
		}
		cout << "--> (" << i << "," << j << ")" << nl;
...
void solvethetestcase() {
	...
	vvi rmat;
	rmat.resize(y, vi(x, 0));
	print_mat(gmat, x, y);
	solve_maze(gmat, rmat, 0, 0);
	cout << "-----------" << nl;
	print_mat(rmat, x, y);
}
