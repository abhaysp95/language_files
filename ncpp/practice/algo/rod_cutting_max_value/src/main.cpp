...
vvi t;
int max_profit(const vi& parr, const vi& larr, int size, int len) {
	rloop(i, 0, size) {
		rloop(j, 0, len) {
			if (!i || !j)
				t[i][j] = 0;
				...
				if (larr[i - 1] > j)
					...
					t[i][j] = mx(parr[i - 1] + t[i][j - larr[i - 1]], t[i - 1][j]);
			}
		}
	}
	return t[size][len];
...
void print_matrix(int size, int sum) {
	rloop(i, 0, size) {
		rloop(j, 0, sum) {
			cout << t[i][j] << " ";
		}
...
void solvethetestcase() {
	...
	cin >> lrod;
	t.clear();
	t.resize(larr.size() + 1, vi(lrod + 1, -1));
	cout << max_profit(parr, larr, larr.size(), lrod) << nl;
	print_matrix(larr.size(), lrod);
}
