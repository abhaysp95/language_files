...
int max_point(const vvi& data) {
	int temp{}, sum{};
	loop (i, 0, data[0].size()) {
		temp += data[0][i];
		sum += (data[1][i] - temp);
	}
	return sum;
...
/*vvi transpose(vvi&& vect) {
	vvi t(vect[0].size(), vi());
	loop (i, 0, vect.size()) {
		loop (j, 0, vect[0].size()) {
			t[j].pb(vect[i][j]);  // fills column-wise
		}
	}
	return t;
}*/
...
/*vvi&& transpose(vvi&& vect) {
	...
	return std::move(t);  // you can't do this
}*/

void transpose(vvi& vect) {
	vvi t(vect[0].size(), vi());
	loop (i, 0, vect.size()) {
		loop (j, 0, vect[0].size()) t[j].pb(vect[i][j]);
	}
	vect = std::move(t);
}
...
void solvethetestcase() {
	string row{};
	// first row will be duration, second will be deadline
	vvi data;
	while (getline(cin, row)) {
		if (row == "") break;
		vi temp;
		tokenize(row, temp, ' ');
		data.pb(std::move(temp));
	}
	// transpose
	//transpose(std::move(data));
	transpose(data);
	// sort according to duration
	sort(data.begin(), data.end(), [](vi& d1, vi& d2) {
			return d1[0] < d2[0];
			});
	//transpose(std::move(data));
	transpose(data);
	cout << max_point(data) << nl;
}
