...
/**
  DNF(Dutch National Flag) sort also know as 0,1,2 sort

Logic:
	a. you have the array, you'll make 3 pointers in it, named: low, mid, high
	b. initially, low and mid will be pointing 0 index and high to the last index
	c. you'll start comparing elements of mid pointer, if element in mid is:
		0: swap mid and low element and increment both low and mid pointer
        1: increment mid pointer
        2: if mid is 2 then swap with high pointer and decrement high pointer
	d. base condition will be, when there is no gap between mid and high(mid <= high)
...
void dnf_sort(vi& arr) {
	int low{}, mid{}, high{static_cast<int>(arr.size() - 1)};
	while (high >= mid) {
		if (!arr[mid]) {
			swap(arr, low, mid);
			low++;
