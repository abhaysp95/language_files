...
/**
  logic of count sort
  1. take array input in 'arr'
  2. have a temporary array which will be of size max(arr), and store the
	 frequence of elements with their value as the index for this temporary
	 array
  3. then in the temporary array, have them make them position by, adding
	 previous index element to current index element
  4. reverse iterate the orignal array and for the value from the orignal array
	 picking the position after decrementing it one time and fill it to that
	 element in the got index postion in new array
