...
void swap(vi::iterator x, vi::iterator y) {
	int temp = *x;
	*x = *y;
	*y = temp;
}

void wave_sort(vi& arr) {
	// NOTE: while jumping more than 1 step, always use </>/<=/=> to compare
	// iterators, not != as it may get out-of-range and you'll get seg. faults
	for (vi::iterator iter = arr.begin() + 1; iter < arr.end(); iter += 2) {
		if (*iter > *(iter - 1))
			swap(iter, iter - 1);
