...
void solvethetestcase() {
	...
	int lcs_len = get_lcs(a, b);
	cout << "insertion: " << (b.size() - lcs_len) << ", "
		<< "deletion: " << (a.size() - lcs_len) << nl;
	/**
	  for min # of insertion, deletion to convert string b to string a, do:
	  insertion -> a.size() - lcs_len
	  deletion -> b.size() - lcs_len
	*/
}
