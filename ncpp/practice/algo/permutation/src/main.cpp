...
vvi ans;
void swap(vi& arr, int i, int idx) {
	int temp = arr[i];
	arr[i] = arr[idx];
	arr[idx] = temp;
}
...
// duplicates allowed
void permute_dup(vi& arr, int idx) {
	if (idx == arr.size()) {
		ans.pb(arr);
		return;
	}
	loop (i, idx, (arr.size())) {
		//cout << "i: " << i << ", idx: " << idx << nl;
		swap(arr, i, idx);
		permute_dup(arr, idx + 1);
		//cout << "i: " << i << ", idx: " << idx << nl;
		swap(arr, i, idx);
	}
}

// duplicates not allowed(unique permutation)
void permute_uq(vi& arr, int idx) {
	if (idx == arr.size()) {
		ans.pb(arr);
		return;
	}
	loop(i, idx, (arr.size())) {
		if (i != idx && arr[i] == arr[idx])
			continue;
		swap(arr, i, idx);
		permute_uq(arr, idx + 1);
		swap(arr, i, idx);
	}
}

void print_matrix(const vvi& arr) {
	rep(i, (arr.size())) {
		cout << "{";
		rep(j, (arr[0].size())) {
			cout << arr[i][j];
