...
int lcs(string& a, string& b, int x, int y) {
	...
	return mx(lcs(a, b, x - 1, y), lcs(a, b, x, y - 1));
}

vvi t;

/**
  since, if first string size is 0, there's no common subsequence and if second
  string size is 0 then also there's no common subsequence so, both first row
  and first col of matrix will be initialized with 0
  */
int lcs_mem(string& a, string& b, int x, int y) {
	if (!x || !y)
		return 0;
	if (t[x][y] != -1)
		return t[x][y];
	if (a[x - 1] == b[y - 1])
		return 1 + lcs_mem(a, b, x - 1, y - 1);
	t[x][y] = mx(lcs_mem(a, b, x - 1, y), lcs_mem(a, b, x, y - 1));
	return t[x][y];
}
...
void solvethetestcase() {
	...
	//cout << lcs_mem(a, b, a.size(), b.size()) << nl;
	cout << lcs_tab(a, b) << nl;
}
