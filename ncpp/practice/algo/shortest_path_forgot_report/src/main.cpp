...
int get_shortest_path(int n, int k) {
	...
	// go to forgot city
	//path_sum += min(mat[n - 1][k - 1], mat[k - 1][n - 1]);
	path_sum += mat[n - 1][k - 1];
	// return back to original city
	path_sum += min(mat[k - 1][0], mat[0][k - 1]);
	//for(int i = k - 1; i > 0; i--) {
		//path_sum += min(mat[i][i - 1], mat[i - 1][i]);
	//}
	return path_sum;
...
void solvethetestcase() {
	...
	print_mat(mat);
	cout << "shortest path: " << get_shortest_path(N, K) << nl;
}
