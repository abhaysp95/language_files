...
/**
  To get the number of ways coins from give coin array could be taken to
  achieve given target. You have unlimited supply of coin This problem is
  somewhat similar to problem of subset_sum count with the difference that this
  is unbounded knapsack based(unlimited supply)
  */

vvi t;
int cc_num_ways(const vi& carr, int size, int target) {
	rloop(i, 0, target)
		t[0][i] = 0;
	rloop(i, 0, size)
		t[i][0] = 1;
	rloop(i, 1, size) {
		rloop(j, 1, target) {
			if (carr[i - 1] > j)
				...
				t[i][j] = t[i][j - carr[i - 1]] + t[i - 1][j];
		}
	}
	return t[size][target];
}

void print_mat(int x, int y) {
	rloop(i, 0, x) {
		rloop(j, 0, y)
			cout << t[i][j] << " ";
		br;
...
void solvethetestcase() {
	...
	cout << cc_num_ways(carr, carr.size(), target) << nl;
	print_mat(carr.size(), target);
}
