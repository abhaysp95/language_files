...
bool is_safe(const vvi& b, int x, int y) {
	rep(i, x) {
		if (b[i][y] == 1)  // that column is occupied
			return false;
	}
...
bool place_queens(vvi& b, int row, int size) {
	if (row == size)
		return true;
...
void print_board(const vvi& b, int size) {
	rep(i, size) {
		rep(j, size) {
			cout << b[i][j];
			if (j < size - 1)
...
void solvethetestcase() {
	...
	cin >> x;
	vvi board{};
	board.resize(x, vi(x, 0));
	place_queens(board, 0, x);
	br;
	print_board(board, x);
}
