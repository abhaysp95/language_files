...
vvi t;
pair<int, int> get_max_point_lcs(string& a, string& b) {
	int mxc = -INF;
...
string get_lcs_str(string& a, string& b, pair<int, int>& point) {
	int x = point.fe;
	int y = point.se;
	string res{};
	while (x && y) {
...
void solvethetestcase() {
	...
	pair<int, int> point = get_max_point_lcs(a, b);
	cout << point.fe << ", " << point.se << nl;
	string res = get_lcs_str(a, b, point);
	reverse(all(res));
	cout << res << nl;
}
