...
bool is_anagram(string s1, string s2) {
	using ucimap = unordered_map<char, int>;
	ucimap ump{};
	for_each(s1.begin(), s1.end(), [&ump](const char& x) {
			ucimap::const_iterator citer = ump.find(x);
			if(citer != ump.cend()) ump[x]++;
			else ump[x] = 1;
			});
	for_each(s2.begin(), s2.end(), [&ump](const char& x) {
			ucimap::const_iterator citer = ump.find(x);
			if(citer != ump.cend()) ump[x]--;
			else ump[x] = 1;
			});
...
#define SZ 256
	int arr[SZ];
	//memset(arr, 0, sizeof(arr));  // this is not working in ques. env.
...
void solvethetestcase() {
	string s1{}, s2{};
	cin >> s1 >> s2;
	cout << (is_anagram2(s1, s2) ? "Yes" : "No") << nl;
}
