...
void desired_permutations(string a, char ch, int pos, int l, int r, vector<string>& perm) {
	if(l >= r + 1) {
		if(a[pos] != ch)
			perm.push_back(a);
		return;
	...
	for(int i = l; i <= r; i++) {
		swap(a[l], a[i]);
		desired_permutations(a, ch, pos, l + 1, r, perm);
		swap(a[l], a[i]);
	}
}
...
void solvethetestcase() {
	...
	cin >> in >> c >> pos;
	vector<string> res{};
	// this much parameters were given in question
	desired_permutations(in, c, pos, 0, in.size() - 1, res);
	for(const string& x: res) cout << x << nl;
}
