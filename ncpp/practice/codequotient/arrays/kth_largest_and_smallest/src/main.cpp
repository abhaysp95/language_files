...
int find_kth_largest(int *arr, int size, int k) {
	...
	return arr[size - k];  // change this for kth smallest
}


/** Not doing array sorting, but following the approach of how quick sorting is
 * done */

/** input test case:
  first line->no. of test case
  {
	k
	size of array
	array elements
...
int kth_largest(int arr[], int size, int k) {
	return find_kth_element(arr, 0, size, k);
}
...
void solvethetestcase() {
	...
	cout << find_kth_largest(arr, size, k) << nl;
}
