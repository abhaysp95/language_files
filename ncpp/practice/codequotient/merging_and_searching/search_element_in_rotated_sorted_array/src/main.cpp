...
int search_element(const vi& vec, int left, int right, int key) {
	int mid{};
	while(left <= right) {
		mid = (left + right) / 2;
...
int get_pivot_index(const vi& vec) {
	int left = 0, right = vec.size() - 1;
	int mid{};
	while(left <= right) {
		mid = (left + right) / 2;
		if(mid != vec.size() - 1 && vec[mid] > vec[mid + 1]) return mid;
		else if(mid != 0 && vec[mid - 1] > vec[mid]) return mid - 1;
		else {
			if(vec[left] >= vec[mid]) right = mid - 1;
			else left = mid + 1;
		}
	}
	return -1;
...
void solvethetestcase() {
	...
	getline(cin, input);
	tokenize(input, in, ',');
	int pivot_idx = get_pivot_index(in);
	cout << "pivot index: " << pivot_idx << nl;
	int res = search_element(in, 0, pivot_idx, key);
	if(res == -1) {
		res = search_element(in, pivot_idx + 1, in.size() - 1, key);
	}
	cout << "element index: " << res << nl;
}
