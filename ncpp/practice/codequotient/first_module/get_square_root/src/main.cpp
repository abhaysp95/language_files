...
double squarert(double num) {
	double precision = pow(10, -10);  // numeric_limit<double>::epsilon() was getting too small(so, it became infinite loop)
	double x = num;
	while(fabs(num - (x * x)) > precision) {
		x = (x + (num / x)) / 2;
	}
	return x;
}
...
void solvethetestcase() {
	double num{};
	cin >> num;
	cout << fixed << setprecision(2) << squarert(num) << nl;
}
