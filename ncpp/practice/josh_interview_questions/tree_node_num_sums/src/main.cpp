...
typedef struct _node {
	...
	struct _node *_left;
	struct _node *_right;
	_node(int x): _data{x}, _left{nullptr}, _right{nullptr} {}
} Node;

void insert(Node* & root, int data) {
	if(nullptr == root) root = new Node(data);
	else {
		if(data > root->_data) insert(root->_right, data);
		else if(data < root->_data) insert(root->_left, data);
	}
}
...
int get_num(const vi& vec) {
	int num{};
	rep(i, vec.size()) num += (vec[i] * pow(10, i));
	return num;
}

int print_numbers_from_tree(Node* root, vi path) {
	...
	return print_numbers_from_tree(root->_left, path) + print_numbers_from_tree(root->_right, path);
}
...
void solvethetestcase() {
	...
	getline(cin, in);
	tokenize(in, input, ',');
	Node* root = new Node(input.at(0));
	loop(i, 1, input.size()) insert(root, input[i]);
	inorder(root);
	cout << nl << print_numbers_from_tree(root, vi()) << nl;
}
