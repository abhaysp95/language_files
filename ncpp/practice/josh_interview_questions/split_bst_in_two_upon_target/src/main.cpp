...
typedef struct _node {
	...
	_node(int x): data{x}, left{nullptr}, right{nullptr} {}
} Node;
...
pair<Node*, Node*> split_bst(Node* root, int target) {
	if(root == nullptr) return pair<Node*, Node*>{nullptr, nullptr};
	pair<Node*, Node*> roots{};
	if(target >= root->data) {
		roots = split_bst(root->right, target);
		root->right = roots.first;
		roots.first = root;
	}
	else if(target < root->data) {
		roots = split_bst(root->left, target);
...
void solvethetestcase() {
	...
	cin >> target;
	Node *root{nullptr};
	for(const int& x: input) insert(root, x);
	inorder(root);
	br;
	pair<Node*, Node*> roots = split_bst(root, target);
	cout << "From root1: " << nl;
	inorder(roots.first);
	br;
	cout << "From root2: " << nl;
	inorder(roots.second);
	br;
}
