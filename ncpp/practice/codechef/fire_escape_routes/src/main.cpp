...
/** tell the no. of routes which have to be made in case of fire and in how
 * many a group leader can be chosen for each configuration */
...
size_t dfs(int node) {
	visited[node] = 1;
	size_t size{};
	for(const int& x: nodes[node]) if(!visited[x]) size += dfs(x);
	return 1 + size;  // return including current node too
}
...
void solvethetestcase() {
	...
	int escape_route{}, team_leader{1};
	for(int i = 1; i <= n; i++) {
		if(!visited[i]) {
			team_leader *= dfs(i);
			escape_route++;
	...
	cout << "Escape routes: " << escape_route << nl;
	cout << "No. of ways to chose leader: " << team_leader << nl;
}
