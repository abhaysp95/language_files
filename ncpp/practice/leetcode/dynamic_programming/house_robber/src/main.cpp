...
int solve_recursive(const vi& vec, int n) {
	cout << "==> calls: " << n << nl;
	if(n == 0) return vec[0];
	if(n == 1) return max(vec[0], vec[1]);
	int max_v = max(vec[n] + solve_recursive(vec, n - 2),
			solve_recursive(vec, n - 1));
	cout << "took: " << max_v << ", n: " << n << nl;
	//return max(vec[n] + solve_recursive(vec, n - 2),
			//solve_recursive(vec, n - 1));
	return max_v;
}
...
int solve_tabulation(const vi& vec) {
	t.clear();
	t.resize(10001, -1);
	t[0] = 0, t[1] = vec[0];
	for(size_t i = 2; i <= vec.size(); i++) {
		t[i] = max(vec[i - 1] + t[i - 2], t[i - 1]);
	}
	//for(size_t i = 0; i <= vec.size(); i++) cout << t[i] << ' ';
	//br;
	return t[vec.size()];
}
...
void solvethetestcase() {
	...
	getline(cin, input);
	tokenize(input, in, ',');
	//cout << solve_recursive(in, in.size() - 1) << nl;
	cout << solve_tabulation(in) << nl;
}
