...
// this is the simpler version where you have to just buy and sell one time only

int solve(const vi& prices) {
	int min_v{prices[0]}, max_p{0};
	for(size_t i = 1; i < prices.size(); i++) {
		if(min_v > prices[i]) min_v = prices[i];
		if(max_p < (prices[i] - min_v)) max_p = prices[i] - min_v;
	}
	return max_p;
}
