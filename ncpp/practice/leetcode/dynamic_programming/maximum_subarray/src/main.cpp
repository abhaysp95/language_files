...
// kadane's approach
int solve_kadane(const vi& nums) {
	int cmax{0}, mmax{INT_MIN};
	for(int i = 0; i < nums.size(); i++) {
		cmax += nums[i];
		if(mmax < cmax) mmax = cmax;
		if(cmax < 0) cmax = 0;
	}
	return mmax;
}

int max_sum_cross(const vi& nums, int start, int mid, int end) {
...
int solve(const vi& nums, int start, int end) {
	if(start == end) return nums[start];
	int mid = (start + end) / 2;
	return max(max(solve(nums, start, mid),
			solve(nums, mid + 1, end)),
			max_sum_cross(nums, start, mid, end));
}

int divide_and_conquer_approach(const vi& nums) {
...
void solvethetestcase() {
	...
	getline(cin, input);
	tokenize(input, in, ',');
	//cout << solve_kadane(in) << nl;
	cout << divide_and_conquer_approach(in) << nl;
}
