...
long bin_search(const vi& vec, long left, long right, int target) {
	while(left <= right) {
...
long search(const vi& vec, int t) {
	if(vec.empty()) return -1;
	size_t sz = vec.size();
	long left = 0, right = sz - 1, pivot = -1;
	while(left <= right) {
		long mid = (left + right) / 2;
		//cout << "[mid, left, right]: " << mid << ", " << left << ", " << right << nl;
	...
	//cout << "pivot: " << pivot << nl;
	if(pivot != -1) {
		long res = bin_search(vec, 0, pivot, t);
		if(res == -1) {
			res = bin_search(vec, pivot + 1, sz - 1, t);
		}
		return res;
	}
	return -1;
...
void solvethetestcase() {
	...
	int t{};
	cin >> t, cin.ignore();
	//cout << "input: " << input << ", " << t << nl;
	cout << "res: " << search(in, t) << nl;
}
