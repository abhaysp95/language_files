...
vi top_k_frequent(vi& nums, int k) {
	map<int, int> m{};
	vi res{};
	for_each(nums.begin(), nums.end(), [&](const int& x){
				m[x]++;  // values are already initialized to their respective data types
			});
	using mypair = pair<int, int>;
	vector<mypair> sm(begin(m), end(m));
	sort(begin(sm), end(sm), [](const mypair& a, const mypair& b){ return a.second > b.second; });
	while(k--) {
		res.push_back(sm[k].first);
	}
	return res;
}
