...
using matiter = vvi::const_iterator;

bool bin_search(const matiter iter, int target) {
	if(iter->empty()) return false;
	//copy(iter->begin(), iter->end(), ostream_iterator<int>(cout, ", "));
	//br;
	long left = 0, right = iter->size() - 1;
	while(left <= right) {
		long mid = (left + right) / 2;
		//cout << "[l, m, r]: " << left << ", " << mid << ", " << right << nl;
		if(iter->at(mid) == target) return true;
		else {
			if(iter->at(mid) > target) right = mid - 1;
			else left = mid + 1;
		}
	}
	return false;
...
bool search_matrix(const vvi& matrix, int target) {
	if(matrix.empty()) return false;
	size_t csz = matrix[0].size();
	for(matiter citer = matrix.begin(); citer != matrix.cend(); citer++) {
		if(target < citer->at(0)) return false;
		else if(target > citer->at(csz - 1)) continue;
		else if(bin_search(citer, target)) return true;
	}
	return false;
}
...
void solvethetestcase() {
	...
	while(getline(cin, input)) {
		vi in{};
		tokenize(input, in, ',');
		mat.push_back(move(in));
	}
	cout << (search_matrix(mat, target) ? "Yes" : "No") << nl;
}
