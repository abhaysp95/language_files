...
vvi merge_intervals(vvi& mat) {
	vector<vector<int>> res;
	if(mat.empty()) return res;
	// sort according to start time of interval
	sort(mat.begin(), mat.end(), [](const vi& v1, const vi& v2) {
	...
	for(size_t i = 0; i < msz; i++) {
		res.push_back(vi(2));
		if(i < msz - 1 && mat[i][1] > mat[i + 1][0]) {  // if overlap
			res[curr][0] = min(mat[i][0], mat[i + 1][0]);
			res[curr][1] = max(mat[i][0], mat[i + 1][1]);
			start = i, curr++;
			break;
		}
		else {
	...
	cout << "start: " << start << nl;
	if(!res.empty()) {
		for(size_t i = start + 2; i < msz; i++) {
			res.push_back(vi(2));
			if(res[curr][1] > mat[i][0]) {  // overlap
				res[curr][0] = min(res[curr][0], mat[i][0]);
				res[curr][1] = max(res[curr][1], mat[i][1]);
				curr++;
			}
			else {
				res[curr][0] = mat[i][0];
