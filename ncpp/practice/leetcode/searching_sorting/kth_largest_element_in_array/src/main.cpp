...
int find_kth_largest(const vi& vec, int k) {
	using mmap = map<int, int>;
	mmap mp{};
	for_each(vec.begin(), vec.end(), [&mp](const int& x) {
				mp[x]++;
			});
	long freq{};
	for(mmap::const_reverse_iterator criter = mp.crbegin(); criter != mp.crend(); criter++) {
		freq += criter->second;
		if(freq == k) return criter->first;
	}
	return -1;
}
