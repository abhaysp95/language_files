...
int find_peak_element(const vi& vec) {
	const size_t sz = vec.size();
	long left{}, right = sz - 1;
	while(left <= right) {
		long mid = (left + right) / 2;
		if(mid == 0) {
			if(mid + 1 == sz) return mid;
			else if(mid + 1 < sz && vec[mid] > vec[mid + 1]) return mid;
		}
		else if(mid == sz - 1 && vec[mid] > vec[mid - 1]) return mid;
		else if(vec[mid] > vec[mid - 1] && vec[mid] > vec[mid + 1]) return mid;
		else {
			if(vec[mid] < vec[mid + 1]) left = mid + 1;
			else right = mid - 1;
		}
	}
	return -1;
