...
bool can_attend_all_meeetings(vvi& matrix) {
	if(matrix.empty()) return false;
	sort(matrix.begin(), matrix.end(), [](const vi& v1, const vi& v2) {
				return v1.at(0) < v2.at(0);
			});
	print_mat(matrix);
	size_t sz = matrix.size();
	for(size_t i = 0; i < sz - 1; i++) {
		if(matrix[i][1] > matrix[i + 1][0]) return false;
	}
	return true;
}
...
void solvethetestcase() {
	...
	cout << (can_attend_all_meeetings(mat) ? "Yes" : "No") << nl;
}
