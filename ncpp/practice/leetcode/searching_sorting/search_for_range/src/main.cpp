...
// it is already told that array is sorted
mypair search_range(const vi& in, int t) {
	mypair res{-1, -1};
	long l{}, r = in.size() - 1;
	long m{};
	while(l < r) {
		m = (l + r) / 2;
		if(in[m] == t) break;
		else if(in[m] > t) r = m - 1;
		else l = m + 1;
	}
	//cout << "mid: " << m << '\n';
	if(l < r) {  // element found
		l = m, r = m;
		while((l - 1) >= 0 && in[l - 1] == t) l--;
		res.first = l;
		while((r + 1) < in.size() && in[r + 1] == t) r++;
		res.second = r;
	}
	return res;
}
...
// using c++ algorithm
mypair search_range2(const vi& in, int t) {
	mypair res{-1, -1};
	// C++ algorithm 'binary_search' returns bool and not index position
	vi::const_iterator cleft = lower_bound(in.cbegin(), in.cend(), t);
	if(cleft != in.cend() && *cleft == t) {
		res.first = cleft - in.cbegin();
		vi::const_iterator cright = upper_bound(cleft, in.cend(), t);
		res.second = cright - in.cbegin() - 1;
	}
	return res;
}
