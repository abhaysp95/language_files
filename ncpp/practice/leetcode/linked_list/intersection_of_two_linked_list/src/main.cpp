...
ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
	ListNode *f{headA}, *s{headB}, *mxl{nullptr}, *mnl{nullptr};
	if(headA == nullptr || headB == nullptr) return nullptr;
	int sza{}, szb{};
	while(f != nullptr) sza++, f = f->next;
	while(s != nullptr) szb++, s = s->next;
	if(sza >= szb) mxl = headA, mnl = headB;
	else mxl = headB, mnl = headA;
	// cout << sza << ',' << szb << ',' << fabs(sza - szb) << '\n';
	for(size_t i = 0; i < fabs(sza - szb); i++) {
		cout << mxl->val << ' ';
		mxl = mxl->next;
	}
	while(mxl != nullptr && mnl != nullptr) {
		if(mxl == mnl) return mxl;
		mxl = mxl->next;
		mnl = mnl->next;
	}
	return nullptr;
...
ListNode *getIntersectionNode2(ListNode *headA, ListNode *headB) {
	...
	while (l1 != nullptr && l2 != nullptr && l1 != l2) {
		...
		if(l1->next == nullptr) l1 = headB;
		else if(l2->next == nullptr) l2 = headA;
	}

	return l1;  // or you can return l2(or probably nullptr, doesn't matter cause no intersection found)
}
