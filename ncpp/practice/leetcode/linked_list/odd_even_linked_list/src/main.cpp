...
typedef struct _node {
	int val;
	struct _node *next;
	_node(): val{}, next{nullptr} {}
	_node(int x): val{x}, next{nullptr} {}
	_node(int x, _node *n): val{x}, next{n} {}
} ListNode;

void insert_vec(ListNode* & head, const vi& vec) {
...
ListNode* odd_even_list(ListNode *head) {
	...
	vector<int> temp{};  // to save even indices values
	ListNode *f{head}, *s{head};
	if(head->next != nullptr) {
		f = head->next;
		if(f->next != nullptr) s = f->next;
		else return head;
	} else return head;
	...
	while(s != nullptr) {
		...
		f = f->next;
		if(s->next != nullptr) {
			temp.push_back(s->next->val);
			s = s->next->next;
		}
		else break;  // it's the last node
	}
