...
vi plus_one(vi& digits) {
	list<int> lst;
	int carry = 0, temp = 0;
	vi::reverse_iterator riter = digits.rbegin();
	while(riter != digits.rend()) {
		temp = *riter + carry;
		if(riter == digits.rbegin()) temp += 1;
		carry = temp / 10;
		lst.push_front(temp % 10);
		riter++;
	}
	if(carry) lst.push_front(carry);
	return vi(lst.begin(), lst.end());
}
...
void solvethetestcase() {
	...
	getline(cin, input);
	tokenize(input, in, ',');
	for(const int& x: plus_one(in)) cout << x << ' ';
	br;
}
