...
int single_number(vi& in) {
	size_t orig_sum = accumulate(in.begin(), in.end(), 0);
	size_t unq_sum = 0;
	sort(in.begin(), in.end());
	for(size_t i = 0; i < in.size() - 1; i++) {
		if(in[i] != in[i + 1]) unq_sum += in[i];
	}
	unq_sum += (in[in.size() - 1]);
	return 2 * unq_sum - orig_sum;
}
