...
void move_zeroes(vi& nums) {
	int zptr{}, cptr{};
	while(zptr < nums.size() && cptr < nums.size()) {
		if(0 == nums[cptr]) cptr++;
		else if(0 != nums[zptr]) zptr++;
		else if(0 == nums[zptr] && cptr > zptr) {
			swap(nums[zptr], nums[cptr]);
			zptr++, cptr++;
		}
		else cptr++;
	}
