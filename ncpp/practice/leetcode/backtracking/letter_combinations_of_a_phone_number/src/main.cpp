...
static const unordered_map<char, string> keypad = {
	{'2', "abc"},
	{'3', "def"},
	{'4', "ghi"},
	{'5', "jkl"},
...
void get_combination(const string& digits, vector<string>& res, string str, int i) {
	if(str.size() == digits.size()) {
		res.push_back(str);
		return;
	}
	unordered_map<char, string>::const_iterator citer{};
	if((citer = keypad.find(digits[i])) != keypad.end()) {
		const string& temp = citer->second;
		for_each(temp.begin(), temp.end(), [&, str, i](const char& x) {
				// well, you can also just use [&], since this lambda isn't
				// doing any permanent change
				get_combination(digits, res, str + x, i + 1);
				});
	}
	else exit(1);
}
...
vector<string> letter_combinations(const string& digits) {
	...
	//for_each(digits.begin(), digits.end(), [&](const char& x) {
			//unordered_map<char, string>::const_iterator citer{};
			//if((citer = keypad.find(x)) != keypad.end()) initial_string.push_back((citer->second)[0]);
			//});
	//res.push_back(initial_string);
	get_combination(digits, res, "", 0);
	return res;
}
...
void solvethetestcase() {
	string input{};
	cin >> input;
	vector<string> res = letter_combinations(input);
