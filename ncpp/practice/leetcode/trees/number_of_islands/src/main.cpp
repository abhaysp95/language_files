...
using vbsz = vb::size_type;
using vvbsz = vvb::size_type;
void solve(vvbsz i, vbsz j, vvc& grid, vvb& visited) {
	visited[i][j] = false;
	if((i + 1 < grid.size()) && (grid[i + 1][j] == '1') && visited[i + 1][j]) solve(i + 1, j, grid, visited);
	if((j > 0) && (grid[i][j - 1] == '1') && visited[i][j - 1]) solve(i, j - 1, grid, visited);
	if((j + 1 < grid[0].size()) && (grid[i][j + 1] == '1') && visited[i][j + 1]) solve(i, j + 1, grid, visited);
	if((i > 0) && (grid[i - 1][j] == '1') && visited[i - 1][j]) solve(i - 1, j, grid, visited);
}

int num_islands(vvc& grid) {
	int res{};
	vvb visited(grid.size(), vb(grid[0].size(), true));
	vvbsz m{grid.size()};
	vbsz n{grid[0].size()};
	for(vvbsz i = 0; i < m; ++i) {
		for(vbsz j = 0; j < n; ++j) {
...
void solvethetestcase() {
	...
	while(getline(cin, input)) {
		...
		vc temp{};
		tokenize(input, temp, ',');
		grid.pb(std::move(temp));
	}
	print_mat(grid);
	cout << num_islands(grid) << nl;
}
