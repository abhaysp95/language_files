...
class Debugger
{
	...
	public:
		explicit Debugger(const std::string& _sep = ", "):
			_first{true}, _seperator{_sep} {}

		template<typename ObjectType>
		Debugger& operator,(const ObjectType& v)
		{
			if(!this->_first) std::cerr << this->_seperator;
			std::cerr << v;
			this->_first = false;
			return *this;
		}
...
// make below templates more generic for set/vector/queue/deque etc.
template<typename T>
inline std::ostream&
...
{
	typename std::set<T>::const_iterator citer = s.cbegin();
	os << "[";
	if(citer != s.cend()) os << *citer++;
	while(citer != s.cend()) os << ", " << *citer++;
	return os << "]";
}

template<typename T1, typename T2>
inline std::ostream&
operator<<(std::ostream& os, const map<T1, T2>& m)
{
	typename map<T1, T2>::const_iterator citer = m.cbegin();
	os << "[";
	if(citer != m.cend()) os << *citer++;
	while(citer != m.cend()) os << ", " << *citer++;
	return os << "]";
}
