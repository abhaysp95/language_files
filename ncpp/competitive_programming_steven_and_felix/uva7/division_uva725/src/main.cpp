...
signed main() {
	...
	//FAST_IO;  // comment this line if you're testing out initial approach
...
bool not_matched(int i, int num) {
	bitset<10> val{false};
	if(i < 10000) val.set(0, true);
	while(i) {
		int rem = i % 10;
		i /= 10;
		if(val.test(rem)) return false;
		val.set(rem, true);
	}
	while(num) {
		int rem = num % 10;
...
void solvethetestcase() {
	...
	while(1) {
		...
		for(int i = LOL; i <= UPL; i++) {
			int num = i * N;
			if(num > UPL) break;
			if(not_matched(i, num)) {
				found = true;
				cout << num << " / " << i << " = " << N << nl;
			}
		}
		if(!found) cout << "There are no solutions for " << N << nl;
		br;
	}
} */
...
void solvethetestcase() {
	...
	while(scanf("%d", &N) && N) {
		...
		for(int i = 1234; i < 98765; i++) {
			int num = i * N;
			if(num > 98765) break;
			char nums[10], used[10];
			bool flag1{true};
			memset(used, 0, sizeof(used));
			sprintf(nums, "%05d", i);
			for(int j = 0; nums[j]; j++)
				used[nums[j] - '0']++;
			sprintf(nums, "%05d", num);
			for(int j = 0; nums[j]; j++)
				used[nums[j] - '0']++;
			for(int j = 0; j < 10; j++)
				if(used[j] != 1) flag1 = false;
			if(flag1) {
				flag = true;
				printf("%5d / %05d = %d\n", num, i, N);
			}
