...
long insertion(long n, int m, int i, int j) {
	for(int k = i; k <= j; k++) {
		//n |= (((m & (1 << (k - i))) ? 1 : 0) << k);  // one-liner
		n &= ~(1 << k);  // clear bit
		int gb = m & (1 << (k - i));
		gb = (gb ? 1 : 0);
		n |= (gb << k);
...
long insertion2(long n, int m, int i, int j) {
	// create mask to clear the bits
	int left = (~0) << (j + 1);
	int right = (1 << i) - 1;
	n &= (left | right);  // bits cleared
	return n | (m << i);  // update
}
