...
/** in arithmetic shift, we shift values to the right(same stands true
 * for left shift), by placing new bits to the value of sign bit, it's same as
 * dividing by 2 everytime */
int repeated_arithmetic_shift(int x, int count) {
	for(int i = 0; i < count; i++) x >>= 1;  // right shift one time in every iteration
	return x;
}

/** in logical shift, we shift values to the right(same stands true for left
 * shift), by placing 0(even in MSB too), for +ve number it's same as dividing by
 */
/*int repeated_logical_shift(int x, int count) {
	for(int i = 0; i < count; i++) x >>>= 1;
	return x;
}*/

/** whether that bit in num is on or off, idx should start from 0 */
bool get_bit(int num, int idx) {
	return (0 != (num & (1 << idx)));
}

int set_bit(int num, int idx) {
	return num | (1 << idx);
}

int clear_bit(int num, int idx) {
	return num & (~(1 << idx));
}

/** clear all bits from MSB through idx(inclusive) */
int clear_bits_MSB_through_idx(int num, int idx) {
	return num & ((1 << idx) - 1);
}

/** clear all bits from idx through zero */
int clear_bits_idx_through_zero(int num, int idx) {
	return num & (~((1 << idx) - 1));
}

/** for the above two functions, we can also use, left shift and right shift
 * idx times on -1(is all 1s, with MSB set to 1 too) too. */

int update_bit(int num, int idx, int bit_is_1) {
	int value = bit_is_1 ? 1 : 0;
	return (num & (~(1 << idx))) | (value << idx);  // clear and set bit
}
