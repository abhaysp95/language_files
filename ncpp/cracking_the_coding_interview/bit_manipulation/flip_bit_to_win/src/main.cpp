...
int solve(int num) {
	if(0 == ~num) return sizeof(num) * 8;
	int old_count{}, current_count{}, max_count = -INF;
	bool is_zero_bit_flipped{false};
	do {
		int last_bit = num & 1;
		if(0 == last_bit) {
			if(!is_zero_bit_flipped) {
				old_count = current_count;
				current_count = 0;
				is_zero_bit_flipped = true;
			}
			else {
				max_count = mx(max_count, old_count + current_count);
				// remove the 0 you counted as 1 previously
				old_count = current_count - 1;
				current_count = 0;
				//is_zero_bit_flipped = false;
			}
		}
		current_count++;
		num >>= 1;
...
/** somewhat similar approach as above */
int solve2(int num) {
	if(0 == ~num) return sizeof(num) * 8;
	int max_count = -INF, previous_count{0}, current_count{0};
	do {
		if(1 == (num & 1)) current_count++;
		else {
			/** update previous_count to current_count, if only one bit next is
			* 0, no more */
			previous_count = (0 == (num & 2) ? 0 : current_count);
			current_count = 0;
		}
		max_count = mx(max_count, current_count + previous_count + 1);
		num >>= 1;
	} while(0 != num);
	return max_count;
}
