...
string solve(double num) {
	if(1 <= num || 0 >= num) return "ERROR";
	stringstream ss;
	ss << ".";
	int count{};
	while(0.0 < num) {  // that's the limit told in question
...
string solve2(double num) {
	...
	if(32 < ss.str().size() - 1) return "ERROR";
	return ss.str();
}
