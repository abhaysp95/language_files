...
int get_next_max(int num) {
	int temp{num};
	int count_1 = get_count_of_1(num);
	while(true) {
		int temp_num_count_1 = get_count_of_1(++num);
		if(count_1 == temp_num_count_1 && temp < num) break;
	}
	return num;
}

int get_nex_min(int num) {
	if (0 == num) return 0;
	int temp{num};
	int count_1 = get_count_of_1(num);
	while(num > 0) {
...
int get_min_next(int num) {
	// flip rightmost non-trailing zero
	int pos{1};
	int num_of_one = (0 != (num & (1 << 0)) ? 1 : 0);
	while(true) {
		if(0 != (num & (1 << pos))) num_of_one++;
		// found a zero, whose previous bit is 1
		if(0 == (num & (1 << pos)) && 0 != (num & (1 << (pos - 1)))) break;
		pos++;
	}
	num |= (1 << pos);
	// clear all 1 bits right to pos
	num &= (~((1 << (pos)) - 1));
	// put num_of_one - 1 1s to rightmost part of the number
	num |= ((1 << (num_of_one - 1)) - 1);
	return num;
}

int get_max_prev(int num) {
	int pos{1};
	int num_of_one = (0 != (num & (1 << 0)) ? 1 : 0);
	while(true) {
		if(0 != (num & (1 << pos))) num_of_one++;
	...
	num &= (~((1 << (pos + 1)) - 1));
	num |= (((1 << num_of_one) - 1) << (pos - num_of_one));
	return num;
}
...
void solvethetestcase() {
	int num{};
	cin >> num;
	//cout << get_next_max(num) << nl;
	//cout << get_nex_min(num) << nl;
	cout << get_min_next(num) << nl;
	cout << get_max_prev(num) << nl;
}
