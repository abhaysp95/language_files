...
int num_of_flips(int from, int to) {
	int diff{};
	while(0 != from && 0 != to) {
		if((from & 1) != (to & 1)) diff++;
		from >>= 1;
...
int num_of_flips2(int from, int to) {
	return count_one(from ^ to);
}

/** one more way */

int num_of_flips3(int from, int to) {
	int res{0};
	for(int i = from ^ to; i > 0; i &= (i - 1)) res++;
	return res;
}
...
void solvethetestcase() {
	int from{}, to{};
	cin >> from >> to;
	//cout << num_of_flips(from, to) << nl;
	cout << num_of_flips3(from, to) << nl;
}
