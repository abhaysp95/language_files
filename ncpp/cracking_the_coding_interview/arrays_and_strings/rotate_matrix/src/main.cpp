...
/** rotate matrix 90 degree in place, anti-clockwise rotation */
void rotate_90(vvi& t) {
	if(0 == t.size() || t.size() != t[0].size()) return;
	int n = t.size();
	for(int row = 0; row < n / 2; row++) {
		...
		for(int i = first; i < last; i++) {
			...
			t[first][i] = t[i][last];
			// bottom -> right
			t[i][last] = t[last][last - offset];
			// left -> bottom
			t[last][last - offset] = t[last - offset][first];
			// top -> left
			t[last - offset][first] = temp;
		}
	}
}
