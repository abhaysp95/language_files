...
inline void print_mat(const vvi& matrix) {
	rep(i, matrix.size()) {
		rep(j, matrix[0].size()) cout << matrix[i][j] << ' ';
		br;
	}
}
...
void nullify_row(vvi& t, int idx) {
	for(int i = 0; i < t[0].size(); i++) t[idx][i] = 0;
}

void nullify_column(vvi& t, int idx) {
	for(int i = 0; i < t.size(); i++) t[i][idx] = 0;
}

void solve(vvi& t) {
	bool frhz{false}, fchz{false};
	...
	for(int i = 0; i < t.size(); i++) {
		if(0 == t[i][0]) nullify_row(t, i);
	}
	for(int i = 0; i < t[0].size(); i++) {
		if(0 == t[0][i]) nullify_column(t, i);
	}
	if(frhz) nullify_row(t, 0);
	if(fchz) nullify_column(t, 0);
}
