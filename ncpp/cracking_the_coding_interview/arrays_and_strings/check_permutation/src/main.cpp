...
bool solve( const string& in1, const string& in2 ) {
	...
	string t;
	for( const char& x: in1 ) {
		if( find( t.cbegin(), t.cend(), x ) == t.cend() ) t += x;
	}
	if( t.size() != in2.size() ) return false;
	for( const char& x: t ) {
		string::const_iterator citer = find( in2.begin(), in2.end(), x );
		if( citer == in2.cend() ) return false;
	}
	return true;
}
...
bool solve3( const string& in1, const string& in2 ) {
	if( in1.size() != in2.size() ) return false;
	array<int, 128> l{ 0 };  // assumption for ASCII
	for( const char& x: in1 )
		l[x]++;
	for( const char& x: in2 ) {
		l[x]--;
		if( 0 > l[x] ) return false;
	}
	return true;
}
...
void solvethetestcase() {
	string in1{}, in2{};
	cin >> in1 >> in2;
	cout << ( solve3( in1, in2 ) ? "TRUE" : "FALSE" ) << nl;
}
