...
bool solve( const string& in ) {
	...
	for( const char& x: in ) {
		char c;
		if( isalpha( x ) ) c = tolower( x );
		else if( x != ' ' ) c = x;
		else continue;
		// value in map will be initialized already( by given data type )
		m[c]++;
		if( 1 == m[c] % 2 ) odd_count++;
		else odd_count--;
	}
	return 1 >= odd_count;
}

/** solve2 related functions */

int toggle( int bitvector, int idx ) {
	if( idx < 0 ) return bitvector;
	int mask = 1 << idx;
	if( 0 == ( bitvector & mask ) ) bitvector |= mask;
	else bitvector &= ~mask;
	return bitvector;
}

int create_bit_vector( const string& in ) {
	int bitvector = 0;
	for( const char& ch: in ) {
		char c{};
		if( isalpha( ch ) ) c = tolower( ch );
		else c = ch;
		if( 'a' <= c && 'z' >= c ) {
			int x = c - 'a';
...
bool solve2( const string& in ) {
	int bitvector = create_bit_vector( in );
