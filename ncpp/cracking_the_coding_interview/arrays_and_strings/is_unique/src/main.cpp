...
bool with_bitset( const string& in ) {
	// size of 128-char alphabet
	if( 128 < in.size() ) return false;
	bitset<26> bs;
	for( const char& x: in ) {
		char c = tolower( x );
		int idx = c - 97;
...
bool with_bit_manipulation( const string& in ) {
	if( 128 < in.size() ) return false;
	int checker = 0;
	for( const char& x: in ) {
		if( checker & ( 1 << ( x - 'a' ) ) ) return false;
		checker |= ( 1 << ( x - 'a' ) );  // update
	}
	return true;
}

/** if not using additional data structure, we can compare each char with every
 * other char in string, this may take approx o(n^2) time, if not you can sort
 * the string and then find duplicate */

void solvethetestcase() {
	string in{};
	cin >> in;
	bool status = with_bit_manipulation( in );
	cout << ( status ? "TRUE" : "FALSE" ) << nl;
}
