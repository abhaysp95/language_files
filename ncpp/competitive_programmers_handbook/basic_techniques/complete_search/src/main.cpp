...
void print_subset(const vi& vec = subset) {
	for (const int& x: vec)
		cout << x << " ";
	br;
}

// this recursive function will get all the subset for (0 to n -1)
void search(int k) {
	if (k == n) {
		cout << k << ", ";
		print_subset();
...
// if you have custom input, you can use this bit_subset as
void bit_subset_custom() {
	vi subs{5, 6, 7, 8};
	vi::size_type ssize = subs.size();
	rep(i, (1 << ssize)) {
		vi temp;
		rep(j, ssize) {
			if (i & (1 << j))
...
void permutation_recursive(vi& arr, int k) {
	if (k == arr.size()) {
		print_subset(arr);
		return;
	}
	loop(i, k, (arr.size())) {
		if ((i != k) && (arr[i] == arr[k]))
			continue;
		swap(arr[i], arr[k]);
		permutation_recursive(arr, k + 1);
		swap(arr[i], arr[k]);
	}
}

// using C++ STL for generating permuation
void permutation_stl(vi& arr) {
	do {
		print_subset(arr);
